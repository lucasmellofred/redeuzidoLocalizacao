...
tk.Label(root, text="Nome do host do banco de dados:").grid(row=0, column=0, padx=10, pady=10)
ipv4_entry = tk.Entry(root, width=50)
ipv4_entry.grid(row=0, column=1, padx=10, pady=10)

tk.Label(root, text="Porta do Firebird:").grid(row=1, column=0, padx=10, pady=10)
port_entry = tk.Entry(root, width=50)
port_entry.grid(row=1, column=1, padx=10, pady=10)

tk.Label(root, text="Arquivo do Banco de Dados:").grid(row=2, column=0, padx=10, pady=10)
db_entry = tk.Entry(root, width=50)
db_entry.grid(row=2, column=1, padx=10, pady=10)
tk.Button(root, text="Selecionar Banco de Dados", command=select_database).grid(row=2, column=2, padx=10, pady=10)

tk.Label(root, text="Arquivo XLSX:").grid(row=3, column=0, padx=10, pady=10)
file_entry = tk.Entry(root, width=50)
file_entry.grid(row=3, column=1, padx=10, pady=10)
tk.Button(root, text="Selecionar Arquivo", command=select_file).grid(row=3, column=2, padx=10, pady=10)

tk.Button(root, text="Carregar Dados", command=load_data).grid(row=4, column=0, columnspan=3, pady=20)

# Adicionar a área de log com barra de rolagem
...
def show_confirmation(modifications):
    ...
    def on_confirm():
        update_database(modifications)
        confirm_window.destroy()
        notebook.select(1)  # Alterna para a aba 'Execução'

    tk.Button(confirm_window, text="Cancelar", command=confirm_window.destroy).pack(side=tk.LEFT, padx=10, pady=10)
    tk.Button(confirm_window, text="Confirmar", command=on_confirm).pack(side=tk.RIGHT, padx=10, pady=10)
...
ttk.Label(frame1, text="Host do banco de dados:").grid(row=0, column=0, sticky=tk.W)
ipv4_entry = ttk.Entry(frame1, width=20)
ipv4_entry.grid(row=0, column=1, padx=0, pady=5)

ttk.Label(frame1, text="Porta do Firebird:").grid(row=0, column=2, sticky=tk.W)
port_entry = ttk.Entry(frame1, width=10)
port_entry.grid(row=0, column=3, padx=0, pady=5)
        ...
        for reduzido, localizacoes in localizacao_dict.items():
            ...
            if result:
                ...
                modificacoes.append({
                    'reduzido': reduzido,
                    'descricao': descricao,
                    'localizacao_antiga': localizacao_antiga,
                    'localizacao_novo': localizacao_final
                })
...
# Frames para cada aba
aba_config = ttk.Frame(notebook, width=600, height=600)
aba_execucao = ttk.Frame(notebook, width=600, height=600)
notebook.add(aba_config, text='Configurações')
notebook.add(aba_execucao, text='Execução')
